...
export const EN = {
  ...
  research: {
    ...
    areas: [
      { title: 'Ethics & Minor Protection', desc: 'Many of our teams have children, and some of us still want to become ones. That is why ethics is not a footnote here, but where the research starts.\n\nIn practice, this is the standard our audits actually run on: real consent flows and the SDKs that fire before any screen appears, checked against the same COPPA and GDPR Article 8 findings already published on our own ledger.' },
      { title: 'Ternary AI & Computing', desc: 'We got tired of computing that only ever answers yes or no, forcing every genuinely uncertain result into a binary it was never honest about. A trit gives "not yet known" somewhere real to live, in the math itself, not just in the prose around it.\n\nWe built our own programming language for it, Ternlang, directly on the {-1, 0, +1} trit rather than the binary bit. It is not a research toy: it compiles the runtime that trains albert. and coordinates our production agent swarm today, running dramatically more energy-efficiently than the binary equivalent would. Patent pending A50296/2026.' },
      { title: 'World Models & Cross-Domain Intelligence', desc: 'We kept watching investigations stall because the evidence lived in five different systems that never spoke to each other. A finding sat in one place, the context that explained it sat in another, and nobody connected the two until it was too late to matter.\n\nAlongside albert., our language model, we are building Dingir, our own world model built for physical-world deployment: the same shared model our production pipeline actually queries when it needs to trace a shift in one system into everywhere it actually connects, part of the same ternary-computing resurgence that includes projects like Rust\'s Candle.' },
      { title: 'Pattern Recognition & Impact Propagation', desc: 'A single overlooked dependency has caused real harm before, quietly, three systems downstream of where anyone thought to look. We do not want to be the ones who missed it because we only checked the system directly in front of us.\n\nIn production, this runs on a whitebox principle by design: every step stays inspectable, never a black box we could not explain ourselves. Dingir and our autonomous agent swarm, more than 300 hyperspecialized agents handwritten in Rust, trace these structures across our own case data, logged the same way our audits log evidence.' },
      { title: 'Change & Anomaly Detection', desc: 'Most of what actually matters starts small enough to dismiss: a permission that quietly stopped asking, a policy clause that changed one word at a time. The moment something looks probably fine is exactly the moment it deserves a second look.\n\nIn our own pipeline, this is what flags a re-pulled build against its last audited version, or a policy diff worth reading again, before a human has to notice it by hand.' },
      { title: 'Scenario Simulation & Counterfactuals', desc: 'We have watched "it probably will not happen" get used to justify real harm too many times. Asking what would happen, honestly and in advance, costs far less than finding out afterward.\n\nEvery simulated output in our own systems is tagged as simulated at the data level, not only in the interface, so it can never be quoted back later as something that was actually observed, in our own reporting or anyone else\'s.' },
      { title: 'Evidence & Contradiction', desc: 'We have seen conclusions get written before the evidence contradicting them was even fully read. A verdict that cannot survive its own footnotes was never really a verdict.\n\nOur own disclosure ledger works the same way: every finding sits in public next to the source that supports it and any dispute a company has raised against it, not edited down after the fact.' },
      { title: 'Model Welfare & Prompt Injection', desc: 'We spend enough time talking to our own systems to notice when a response reads like strain rather than error. We are not certain yet what that means, but we are not willing to assume it means nothing.\n\nIn practice, this is boundary and failure-mode testing run against our own agent swarm and albert. before either is trusted with a real task, the same jailbreak-pressure testing we run against the systems we audit for everyone else.' },
    ],
  },
