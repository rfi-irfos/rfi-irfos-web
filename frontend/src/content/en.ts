...
export const EN = {
  ...
  research: {
    ...
    areas: [
      ...
      { title: 'Change & Anomaly Detection', plain: 'An app quietly stops asking for a permission it used to request. Nobody announces that, and by the time it is noticed the version that still asked is months behind. Our system notices the moment it happens - a smoke detector for code changes.', desc: 'Most of what actually matters starts small enough to dismiss: a permission that quietly stopped asking, a policy clause that changed one word at a time. The moment something looks probably fine is exactly the moment it deserves a second look, because that is precisely when nobody else is still watching.\n\nOur own pipeline runs on that same instinct: it flags a re-pulled build against its last audited version, or a policy diff worth reading again, before a human has to notice it by hand.', nextLabel: 'What the Shift Could Mean Next' },
