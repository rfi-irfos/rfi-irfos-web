...
export const EN = {
  ...
  research: {
    ...
    areas: [
      ...
      { title: 'Change & Anomaly Detection', plain: 'A shipping corridor develops a delay pattern nobody has explained yet. A flood in one place turns out to share its recorded cause with three others an ocean apart. An app quietly stops asking for a permission it used to request. None of it gets announced - our system notices the moment the pattern shifts, wherever it shifts.', desc: 'Most of what actually matters starts small enough to dismiss: a shipping lane running slower than its own history, a cause code shared by disasters that were never connected before, a permission that quietly stopped asking. The moment something looks probably fine is exactly the moment it deserves a second look, because that is precisely when nobody else is still watching.\n\nOur own pipeline runs on that same instinct at whatever scale the data lives at: a node whose position in the graph drifts far from everything like it, a policy diff worth reading again, a re-pulled build flagged against its last audited version - the same structural check whether it is reading disaster records, sanctions listings, or a stopped permission request, before a human has to notice it by hand.', nextLabel: 'What the Shift Could Mean Next' },
