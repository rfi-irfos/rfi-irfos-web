...
const FAQ_ITEMS: [string, string, string, string][] = [
  ['Who are we and what do we do?', 'Wer sind wir und was machen wir?', 'RFI-IRFOS is a registered Austrian not-for-profit (ZVR 1015608684), tracing back to an interdisciplinary research collective founded 2020 in Vorarlberg. We work across ternary AI systems, world-model and evidence-graph research, and agent evaluation, with albert. and DINGIR as the two projects that tie the rest together, unsolicited security research is one strand of that work, not the whole of it. We stay interdisciplinary on purpose, the problems we care about rarely respect the line between one field and the next.', 'RFI-IRFOS ist ein eingetragenes österreichisches Not-for-Profit (ZVR 1015608684), entstanden 2020 aus einem interdisziplinären Forschungskollektiv in Vorarlberg. Wir arbeiten an ternären KI-Systemen, World-Model- und Evidenzgraph-Forschung sowie Agent-Evaluation, mit albert. und DINGIR als den zwei Projekten, die den Rest zusammenhalten, unaufgeforderte Sicherheitsforschung ist ein Strang dieser Arbeit, nicht die ganze. Wir bleiben absichtlich interdisziplinär, die Probleme, die uns interessieren, halten sich selten an die Grenze zwischen einem Feld und dem nächsten.'],
  ['What is an LLM?', 'Was ist ein LLM?', 'A large language model predicts and generates text from patterns in training data. It is one component we build with and around, not a complete system on its own, which is why we also train albert., our own ternary LLM, instead of treating an LLM as the whole product. A model that only predicts the next token cannot check what it just said against anything real, that check has to come from somewhere else in the system.', 'Ein Large Language Model sagt Text anhand gelernter Muster voraus und erzeugt ihn. Es ist eine Komponente, mit der und um die wir bauen, kein vollständiges System für sich, deshalb trainieren wir mit albert. auch unser eigenes ternäres LLM, statt ein LLM als das ganze Produkt zu behandeln. Ein Modell, das nur das nächste Token vorhersagt, kann nicht selbst prüfen, ob das eben Gesagte stimmt, diese Prüfung muss von woanders im System kommen.'],
  ['What is an agent?', 'Was ist ein Agent?', 'An agent combines a model, tools, and observations to pursue a goal and act on its own. We run agents across very different parts of our work: ingesting and validating claims into DINGIR, our world model, running task suites for client agent evaluations, and cross-checking evidence for the security research we publish, always with a human reviewing before anything ships. We keep that human gate because an agent that can act can also act on a wrong belief, and the cost of that mistake is real, not a retry.', 'Ein Agent verbindet ein Modell, Werkzeuge und Beobachtungen, um selbstständig ein Ziel zu verfolgen und zu handeln. Wir setzen Agenten in sehr unterschiedlichen Teilen unserer Arbeit ein: beim Einspeisen und Prüfen von Behauptungen in DINGIR, unser World Model, bei Aufgabensuiten für Kunden-Agent-Evaluationen, und beim Gegenprüfen von Evidenz für die Sicherheitsforschung, die wir veröffentlichen, immer mit einem Menschen, der prüft, bevor etwas rausgeht. Dieses menschliche Gate behalten wir, weil ein Agent, der handeln kann, auch auf einem falschen Befund handeln kann, und der Preis dafür ist real, kein einfacher Neuversuch.'],
  ['What is a world model?', 'Was ist ein World Model?', 'A world model keeps a structured, evolving picture of entities, relationships, and causes so a claim can be checked against reality instead of just sounding right. Ours is called DINGIR: a graph that learns its own topology with a neural network and stamps every claim with a ternary state, supported, unresolved, or refuted, never a flat yes. We refuse the flat yes on purpose, most systems that claim certainty are just hiding the cases where they were not sure.', 'Ein World Model hält ein strukturiertes, sich veränderndes Bild von Entitäten, Beziehungen und Ursachen bereit, damit eine Behauptung an der Realität geprüft werden kann, statt nur plausibel zu klingen. Unseres heißt DINGIR: ein Graph, der seine Topologie mit einem neuronalen Netz lernt und jede Behauptung mit einem ternären Zustand versieht, bestätigt, ungeklärt oder widerlegt, nie ein flaches Ja. Das flache Ja verweigern wir bewusst, die meisten Systeme, die Gewissheit behaupten, verstecken damit nur die Fälle, in denen sie sich nicht sicher waren.'],
  ['What is a data warehouse?', 'Was ist ein Data Warehouse?', 'A data warehouse pulls information from many sources into one consistent, queryable place. We build ours from the evidence our own research already produces, world-model observations, agent-safety trajectories, app-behaviour data, and offer it as structured, provenance-tracked data through our Data Solutions line, not just kept for internal use. The work of producing that evidence already happened once, we saw no reason to make a customer pay for us to do it again from scratch.', 'Ein Data Warehouse führt Informationen aus vielen Quellen an einem konsistenten, abfragbaren Ort zusammen. Unseres bauen wir aus der Evidenz, die unsere eigene Forschung ohnehin erzeugt, World-Model-Beobachtungen, Agent-Safety-Trajektorien, App-Verhaltensdaten, und bieten sie über unsere Data-Solutions-Linie als strukturierte, herkunftsnachvollziehbare Daten an, nicht nur intern gehalten. Die Arbeit, diese Evidenz zu erzeugen, ist bereits einmal passiert, wir sahen keinen Grund, eine Kundschaft dafür zahlen zu lassen, dass wir sie von Grund auf wiederholen.'],
  ['What do we build?', 'Was bauen wir?', 'Our own stack: a ternary logic runtime built from scratch, not borrowed from binary AI frameworks, albert., our self-trained ternary LLM, DINGIR, our evidence-graph world model, Rust and Axum backend services, and the tooling behind 300+ Android security audits. Same team builds all of it, because a system we understand end to end is one we can actually stand behind when something in it is wrong.', 'Unseren eigenen Stack: eine von Grund auf gebaute ternäre Logik-Runtime, nicht aus binären KI-Frameworks abgeleitet, albert., unser selbst trainiertes ternäres LLM, DINGIR, unser Evidenzgraph-World-Model, Rust- und Axum-Backend-Services, und das Werkzeug hinter über 300 Android-Security-Audits. Dasselbe Team baut alles davon, weil wir nur für ein System, das wir Ende-zu-Ende verstehen, wirklich einstehen können, wenn darin etwas nicht stimmt.'],
  ['What does whitebox mean?', 'Was bedeutet Whitebox?', 'Every step of what we build is traceable and inspectable, not hidden behind a vendor black box. We think of ourselves as the whitebox counterpart to systems like Palantir: the same category of capability, but you can see how a conclusion was reached instead of just being told to trust it. That distinction only means something if we hold ourselves to it too, so the same traceability applies to our own findings, not only to the systems we sell.', 'Jeder Schritt dessen, was wir bauen, ist nachvollziehbar und einsehbar, nicht hinter der Blackbox eines Anbieters versteckt. Wir verstehen uns als die Whitebox-Entsprechung zu Systemen wie Palantir: dieselbe Kategorie an Fähigkeit, aber man sieht, wie eine Schlussfolgerung zustande kam, statt sie nur glauben zu müssen. Dieser Unterschied zählt nur, wenn wir uns selbst daran halten, deshalb gilt dieselbe Nachvollziehbarkeit für unsere eigenen Befunde, nicht nur für die Systeme, die wir verkaufen.'],
  ['Do you keep audit trails?', 'Führt ihr Audit-Trails?', 'Yes, and we check them against tampering, not just log them. Our internal systems anchor audit history outside the database itself, so a restored backup or a rewritten row gets flagged automatically instead of being discovered by accident. A log that only the same database can vouch for is not really independent evidence, it is just a second copy of the same claim.', 'Ja, und wir prüfen sie auch auf Manipulation, nicht nur, dass sie geloggt werden. Unsere internen Systeme verankern die Audit-Historie außerhalb der eigentlichen Datenbank, damit ein wiederhergestelltes Backup oder eine nachträglich veränderte Zeile automatisch auffällt, statt zufällig entdeckt zu werden. Ein Log, für das nur dieselbe Datenbank bürgt, ist keine wirklich unabhängige Evidenz, sondern nur eine zweite Kopie derselben Behauptung.'],
  ['How do you handle personal data?', 'Wie geht ihr mit personenbezogenen Daten um?', 'We agree the handling rules with a client before we touch anything, minimise what we collect, and keep test and research data isolated from anything identifying by default. For the unsolicited security research we publish, we go further, no personal data gets published either, only the technical facts and the company\'s own response, under a fixed embargo, whether they reply or not. Publication was never meant to depend on someone paying us to stay quiet, so it does not.', 'Wir vereinbaren die Umgangsregeln mit einer Kundschaft, bevor wir überhaupt etwas anfassen, minimieren was wir erheben, und halten Test- und Forschungsdaten standardmäßig von allem Identifizierenden getrennt. Bei der unaufgeforderten Sicherheitsforschung, die wir veröffentlichen, gehen wir weiter, es werden auch keine personenbezogenen Daten veröffentlicht, nur die technischen Fakten und die Antwort des Unternehmens, unter einem festen Embargo, egal ob geantwortet wird. Veröffentlichung sollte nie davon abhängen, ob uns jemand fürs Schweigen bezahlt, deshalb tut sie das auch nicht.'],
  ['Is client data published?', 'Werden Kundendaten veröffentlicht?', 'No. A client\'s private data, environments, and results stay private unless they explicitly approve publication. Separately, we also run our own unsolicited security research into public apps, and that follows a different, fixed 90 day disclosure clock that applies the same way regardless of payment, a paying client and an unrelated app vendor are not the same relationship and should not sit under the same rule.', 'Nein. Private Daten, Umgebungen und Ergebnisse einer Kundschaft bleiben privat, außer sie geben eine ausdrückliche Freigabe. Daneben betreiben wir auch eigene unaufgeforderte Sicherheitsforschung zu öffentlichen Apps, die einer anderen, festen 90-Tage-Offenlegungsfrist folgt, die unabhängig von einer Zahlung gleich gilt, eine zahlende Kundschaft und ein unbeteiligter App-Anbieter sind nicht dieselbe Beziehung und sollten nicht derselben Regel unterliegen.'],
  ['Do you build mass-surveillance systems?', 'Baut ihr Massenüberwachung?', 'No. We audit companies that overreach on data collection, we do not build the overreach ourselves. That line holds regardless of who is asking or what is on offer, because the whole reason our audits carry any weight is that we have never been on the other side of one.', 'Nein. Wir prüfen Unternehmen, die bei der Datenerhebung übers Ziel hinausschießen, wir bauen das Übersteigen nicht selbst. Diese Grenze gilt unabhängig davon, wer fragt oder was geboten wird, weil unsere Audits nur deshalb Gewicht haben, weil wir selbst nie auf der anderen Seite eines solchen standen.'],
  ['Do you do social engineering?', 'Macht ihr Social Engineering?', 'No. Every finding in our security disclosures comes from decompiling a public app binary or reading a company\'s own published policy, never from tricking an employee or a support contact into giving something up. That standard extends past disclosures too, we do not manipulate people to get information anywhere in our work, whatever the research question.', 'Nein. Jeder Befund in unseren Sicherheits-Offenlegungen stammt aus dem Dekompilieren einer öffentlichen App oder dem Lesen der eigenen veröffentlichten Richtlinie eines Unternehmens, nie daraus, eine Mitarbeiterin oder einen Support-Kontakt zur Herausgabe von etwas zu bewegen. Dieser Maßstab gilt über Offenlegungen hinaus, wir manipulieren nirgends in unserer Arbeit Menschen, um an Informationen zu kommen, egal um welche Forschungsfrage es geht.'],
  ['Are human rights negotiable?', 'Sind Menschenrechte verhandelbar?', 'No. It is the one line no scope negotiation ever moves, regardless of client, budget, or deadline. A boundary that bends under enough pressure was never really a boundary, it was a starting offer.', 'Nein. Das ist die eine Grenze, die sich bei keiner Scope-Verhandlung verschiebt, egal welche Kundschaft, welches Budget, welche Frist. Eine Grenze, die unter genug Druck nachgibt, war nie wirklich eine Grenze, sondern nur ein Startangebot.'],
  ['What does explainability by design mean?', 'Was bedeutet Explainability by Design?', 'We design in the answer to "why did the system do that" before writing the first line of code, not after an incident forces us to. Every DINGIR claim ships with its own confidence state and source chain for exactly this reason. Bolting on an explanation after the fact usually means reverse-engineering a story that fits, which is not the same thing as knowing why something actually happened.', 'Wir entwerfen die Antwort auf "warum hat das System das gemacht", bevor die erste Zeile Code geschrieben wird, nicht erst, nachdem uns ein Vorfall dazu zwingt. Jede DINGIR-Behauptung trägt genau deshalb ihren eigenen Vertrauenszustand und ihre Quellenkette. Eine Erklärung nachträglich anzuflicken heißt meist, sich im Nachhinein eine passende Geschichte zu suchen, das ist nicht dasselbe wie zu wissen, warum etwas wirklich passiert ist.'],
  ['How does the EU AI Act influence the stack?', 'Wie beeinflusst der EU AI Act den Stack?', 'Articles 13 (transparency), 14 (human oversight), and 15 (accuracy, robustness, cybersecurity) are reference points we design against directly, not compliance items bolted on once a system already ships. Building to a legal article after launch usually means retrofitting logs and disclaimers around decisions the system already made, which is the same shallow fix as after-the-fact explainability.', 'Artikel 13 (Transparenz), 14 (menschliche Aufsicht) und 15 (Genauigkeit, Robustheit, Cybersicherheit) sind Referenzpunkte, gegen die wir direkt entwerfen, keine Compliance-Punkte, die erst nachträglich draufgesetzt werden. Erst nach dem Launch auf einen Gesetzesartikel hinzubauen bedeutet meist, Logs und Disclaimer um bereits getroffene Entscheidungen herumzubauen, derselbe oberflächliche Fix wie nachträgliche Explainability.'],
  ['Can you evaluate an existing agent?', 'Könnt ihr einen bestehenden Agenten evaluieren?', 'Yes. We build task suites around your actual tools, instrument tool use step by step, label failure and safety categories, and hand back a report anchored in real trajectories, not a generic benchmark score. A benchmark score tells you how an agent did on someone else\'s tasks, not on yours, and those are rarely the same thing.', 'Ja. Wir bauen Aufgabensuiten um eure echten Werkzeuge, instrumentieren die Tool-Nutzung Schritt für Schritt, labeln Fehler- und Sicherheitskategorien und liefern einen Bericht, der in echten Trajektorien verankert ist, nicht in einem generischen Benchmark-Score. Ein Benchmark-Score sagt, wie ein Agent bei fremden Aufgaben abgeschnitten hat, nicht bei euren, und das ist selten dasselbe.'],
  ['Can you build a private test environment?', 'Könnt ihr eine private Testumgebung bauen?', 'Yes. Containerised, deterministic resets, controlled test credentials that never touch production, the same setup we use for our own agent-safety evaluations. We hold outside work to the same standard we hold ourselves to, otherwise the word "test" would not mean anything.', 'Ja. Containerisiert, deterministische Resets, kontrollierte Testzugänge, die nie an Produktivsysteme rühren, dasselbe Setup, das wir für unsere eigenen Agent-Safety-Evaluationen nutzen. Wir legen an fremde Arbeit denselben Maßstab an wie an unsere eigene, sonst würde das Wort "Test" nichts bedeuten.'],
  ['What does reproducible mean?', 'Was bedeutet reproduzierbar?', 'Run the same versioned input against the same environment a year from now and get an auditable trail of exactly what changed and why, not just a similar-looking result. A result that cannot be reproduced is an anecdote, however convincing it looked the first time.', 'Denselben versionierten Input auch in einem Jahr wieder gegen dieselbe Umgebung laufen lassen und einen prüfbaren Nachweis bekommen, was sich verändert hat und warum, nicht nur ein ähnlich aussehendes Ergebnis. Ein Ergebnis, das sich nicht reproduzieren lässt, ist eine Anekdote, egal wie überzeugend es beim ersten Mal wirkte.'],
  ['How do you report security findings?', 'Wie meldet ihr Security-Funde?', 'We run a root level investigation into the app binary, not a surface scan, contact the affected company through a coordinated written channel with regulators copied in, and publish the findings after a fixed 90 day embargo whether they ever reply or not. Copying the regulator in from the start means we are never the only ones who know what we found, which is the point.', 'Wir führen eine Untersuchung auf Root-Ebene im App-Binary durch, keine Oberflächenprüfung, kontaktieren das betroffene Unternehmen über einen koordinierten, schriftlichen Kanal mit Aufsichtsbehörden in Kopie, und veröffentlichen die Befunde nach einem festen 90-Tage-Embargo, egal ob je geantwortet wird. Die Behörde von Anfang an in Kopie zu setzen heißt, dass wir nie die Einzigen sind, die wissen, was wir gefunden haben, genau das ist der Punkt.'],
  ['How can an organisation start?', 'Wie kann eine Organisation starten?', 'Send us one concrete objective, constraint, or system you actually want to understand, not a wishlist. We scope the smallest useful first step from there, because a first engagement that tries to prove everything at once usually proves nothing well.', 'Schickt uns ein konkretes Ziel, eine Grenze oder ein System, das ihr wirklich verstehen wollt, keine Wunschliste. Wir definieren von da aus den kleinsten sinnvollen ersten Schritt, weil ein erstes Engagement, das alles auf einmal beweisen will, meist nichts davon richtig beweist.'],
]
