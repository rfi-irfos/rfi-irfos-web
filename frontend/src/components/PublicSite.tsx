...
const AUDIT_HIGHLIGHTS: { target: string; market: string; sev: string; status: string; finding: string; company?: string }[] = [
  ...
  { target: 'Muslim Pro',         market: 'PRIVATE', sev: 'CRITICAL', status: 'WAITING',     finding: 'com.bitsmedia.android.muslimpro. 100M-user prayer-times/Quran app (Bitsmedia Pte Ltd, Singapore). ByteDance/Pangle ad SDK (125 smali classes) and Tencent IMSDK (80 smali classes, including a LocationElement data model) fully compiled into production and undisclosed as data recipients anywhere in the privacy policy — both SDK parents operate under China\'s National Intelligence Law (2017), which affords no legal mechanism to decline a state intelligence request, processing prayer times, fasting records and Quran reading progress for a predominantly Muslim user base. Three BOOT_COMPLETED receivers plus ACCESS_FINE_LOCATION mean location-collection infrastructure activates at device boot, before the app opens and before any consent screen exists. Facebook Audience Network ContentProvider auto-initialises pre-consent, hardcoded Firebase API key, and a 412-class Prebid Mobile RTB integration broadcasts device ID, location and prayer-time context to programmatic ad auction partners with no certificate pinning on any endpoint. Singapore\'s PDPC has no public complaint email — intake is a Singpass-gated portal or phone only, a structural reporting gap for third-party institutional disclosures.' },
