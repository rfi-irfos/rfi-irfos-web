...
function PriceTierCard({ tier, price, hook, highlight, onBuy, onProposal }: {
...
const AUDIT_HIGHLIGHTS: { target: string; market: string; sev: string; status: string; finding: string; company?: string; aliases?: string[] }[] = [
  ...
  { target: 'Parship',           market: 'PRIVATE', sev: 'CRITICAL', status: 'SUBSTANTIVE',  company: 'ParshipMeet Group', finding: 'ParshipMeet Group sibling to Lovoo. TheMeetGroup facial-detection SDK + hardcoded Firebase API key. Two disclosures, two automated customer-service ticket numbers, zero substantive reply from a person for weeks - then a real one. On 2026-07-31, Henning Andersen at ParshipMeet Group confirmed both the Lovoo and Parship reports had been located internally, apologized for the delay, and committed to a review, additionally noting a proper security.txt contact is now published for the group\'s services. — meaning Parship uses a facial-detection SDK and a hard-coded key in the app. After weeks of only automated ticket numbers, an actual person at the company has now taken ownership of the report and committed to reviewing it, and the group has since published a standard security-contact channel for future researchers.' },
  ...
  { target: 'Taxefy',            market: 'PRIVATE', sev: 'CRITICAL', status: 'ESCALATED',     finding: 'Facebook Login on Austrian tax app. Hardcoded production Firebase key. Privacy Sandbox allowAllToAccess="true" - broadest possible advertising data sharing on an app processing income and tax data. Veriff Art.9 video. The CEO\'s response named this pattern for the disclosure series: the Decompile-Destruction Pivot - instead of addressing the hardcoded key or the ad-sharing config, he demanded destruction of our analysis under threat of copyright/UrhG action, dismissed the report as "offenbar KI-generiert," and questioned whether the research was genuine or a sales funnel, all without disputing a single technical finding. RFI-IRFOS declined the destruction demand (a public interest research exception applies to static analysis of a legally distributed binary), the archive stays intact for the duration of the open GDPR matter, and the Austrian DSB remains visible in cc throughout. — meaning This Austrian tax app lets you log in with Facebook, hardcodes a production database key directly in the app, opens advertising data sharing as widely as possible to other apps on your device, and records an Art. 9 video identity check, all on an app handling your income and tax details. Rather than fixing any of it, the company responded by demanding the evidence be destroyed and threatening legal action - a pattern this research series has seen before: pivot to copyright law to avoid answering the actual privacy question.' },
      ...
      <section id="pricing" style={{ padding: '100px 2rem' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 220px), 1fr))', gap: 16, marginBottom: 48 }}>
            {[
              { tier: 'Market Overview',          price: '€2,500',      stripeKey: 'market_overview',  delivery: '14 calendar days.', highlight: true, hook: 'A ten-page, jargon-free map of your sector, in 14 days.', desc: 'You get a clear, plain-language map of your sector delivered within 14 calendar days: who the key players are, how the rules and regulations actually work for them, and where the real openings sit.\n\nIt runs at least ten pages, with no jargon and no two-hundred-slide deck, just the landscape you need to make a decision, written so a founder can read it in one sitting.\n\nWe start the research within a few days of payment and send the finished report inside 14 calendar days.' },
              { tier: 'Competitor Intelligence',    price: '€7,500',      stripeKey: 'competitor_intel', delivery: '14 calendar days.', highlight: false, hook: 'Three to five competitors taken apart: tech, privacy, positioning.', desc: 'You get three to five of your competitors taken apart: what technology they are built on, how they actually treat user privacy, which is often very different from what they claim, how they position themselves, and where they are strategically weak.\n\nThe result is a realistic picture of the field, including the gaps you can exploit and the ones you need to close. This is the homework most teams skip and later regret.\n\nWe deliver within 14 calendar days of payment.' },
              { tier: 'Sector Intelligence Report', price: '€18,000',     stripeKey: 'sector_intel',     delivery: '14 calendar days.', highlight: false, hook: 'Market, regulation, and technology - refreshed every quarter.', desc: 'You get the complete picture of your sector: the market, the regulations, and the technology, with each major player risk exposure spelled out in numbers rather than vague claims.\n\nBecause markets move, this is not a one-off document, you receive a fresh update every three months, so you always know where the sector is heading and which players are exposed.\n\nWe deliver the first report within 14 calendar days of payment, then refresh it every quarter.' },
              { tier: 'Ongoing Intelligence Briefing', price: '€4,500 / mo', stripeKey: 'ongoing_intel', delivery: 'First briefing within 14 calendar days, then monthly.', highlight: false, hook: 'A dedicated analyst watching your competitors, month after month.', desc: 'You get continuous watching of your competitors so you do not have to: a proper briefing every month, and an immediate alert the moment one of them makes a significant move, whether that is a funding round, a pivot, a breach, or a key hire.\n\nOne dedicated analyst who knows your space handles it, so you hear about a competitor big launch on the day it happens, not after.\n\nWe assign your analyst within a week of payment and the first briefing lands at the end of month one.' },
            ].map((t, i) => (
              <Reveal key={t.tier} delay={i % 4} from={(['left','bottom','right','scale'] as const)[i % 4]}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 220px), 1fr))', gap: 16, marginBottom: 64 }}>
            {[
              { tier: 'Landing Page',   price: '€1,500',  stripeKey: 'web_landing'   as string | null, delivery: '48 hours.', highlight: true, hook: 'A fast, clean single page, live within 48 hours.', desc: 'You get a sharp single-page site built on our own open-source React template, fast, clean, and live within forty-eight hours of payment.\n\nIt is perfect for a launch, an event, or a product that only needs one great page.\n\nNo bloated page builder and no lock-in: you own the code and can take it anywhere.' },
