...
export function PublicSite({ initialSection }: { initialSection?: string | null } = {}) {
  ...
  async function submitTip(e: React.FormEvent) {
    ...
    // the key held server side. If that fails, we fall back to a direct Web3Forms call
    // from here in the browser - Web3Forms' free tier actively rejects server-to-server
    // submissions ("Pro plan required"), so that leg can't be proxied through the
    // backend; it needs VITE_WEB3FORMS_KEY inlined at build time via Vite. Pass it with
    // `fly deploy --build-arg VITE_WEB3FORMS_KEY=<key>` - a `fly secrets set` alone
    // never reaches the Docker build stage and silently no-ops this whole branch.
    try {
      ...
      {checkoutModal && (
        ...
        <div className="rfi-modal-backdrop" style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(4,4,7,0.7)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', display: 'flex', alignItems: mobile ? 'flex-end' : 'center', justifyContent: 'center', padding: mobile ? 0 : '1rem' }}>
          <div className="rfi-modal-panel" style={{
            ...
            <ModalTierBody tier={checkoutModal.tier} price={checkoutModal.price} desc={checkoutModal.desc} delivery={checkoutModal.delivery} mobile={mobile} bullets={checkoutModal.bullets} bring={checkoutModal.bring} mechanism={checkoutModal.mechanism} receive={checkoutModal.receive} />
