...
const AUDIT_HIGHLIGHTS: { target: string; market: string; sev: string; status: string; finding: string; company?: string }[] = [
  ...
  { target: 'Spinwinera app / Spinwinera', market: 'PRIVATE', sev: 'CRITICAL', status: 'RESOLVED', finding: 'RESOLVED — BOTH APPS REMOVED FROM THE PLAY STORE. Same developer account as Merge Chicken (HOME ESSENTIALS & HARDWARE LIMITED, London) re-listed its casino brand directly as two new apps: "Spinwinera app" (com.win.era.appofficial, disguised as a robotic-vacuum/cleaning game) and "Spinwinera" (com.spinwinera.app) — both named after the live real-money casino domain spinwinera.com (Pragmatic Play/BGaming/Evolution live roulette, Bitcoin deposits, no KYC beyond self-declared age, PEGI 3 listing vs. the operator\'s own 18+ site footer). Identified via actor-level linkage (shared account, shared brand, shared evasion pattern already binary-confirmed on the sibling app), not a fresh independent decompile. Reported 2026-07-01, same day Google invited further leads following Merge Chicken\'s removal. Google confirmed both apps removed on 2026-07-07 — 6 days. Second confirmed RFI-IRFOS takedown against the same actor. A related sibling account (ASJ ALL IN ONE SERVICES LIMITED, Roobet/BetOnRed shells) remains reported but unconfirmed.' },
